...
function countFiles(dir: string): number {
  ...
  try {
    ...
    for (const file of files) {
      if (file === 'node_modules' || file === '.next' || file === 'dist' || file === '.git' || file === '.vercel') continue;
      ...
      try {
        ...
        // Ignorar erros de arquivos individuais (ex: permissão)
        continue;
      }
    }
  } catch (error) {
    console.error(`Erro ao contar arquivos em ${dir}:`, error);
  }
  return count;
}

export default function Home() {
  // Tentar encontrar a pasta _sites de forma resiliente
  const cwd = process.cwd();
  let sitesPath = path.join(cwd, "..", "_sites");
  
  if (!existsSync(sitesPath)) {
    sitesPath = path.join(cwd, "_sites");
  }
        ...
        try {
          ...
          if (existsSync(metadataPath)) {
            try {
              ...
              console.error(`Erro ao ler metadata de ${id}:`, e);
            }
          }
          ...
          if (existsSync(packageJsonPath)) {
            try {
              ...
              console.error(`Erro ao ler package.json de ${id}:`, e);
            }
          }
          ...
          console.error(`Erro ao processar site ${id}:`, error);
          return null;
        }
      }).filter((site): site is SiteInfo => site !== null);
    } catch (error) {
      console.error("Erro ao listar diretório de sites:", error);
    }
  }
